# -*- coding: utf-8 -*-
# @Time    : 2018/11/26 13:47
# @Author  : Hann
# @Site    : 
# @File    : urls.py
# @Software: PyCharm

from django.conf.urls import url


from apps.assistants.assistant_refund_jdfbp.views import RefundList, RefundOverView, RefundUpLoad, OperateOrder

urlpatterns = [
    url(r'^list/$', RefundList.as_view(), name='refundlist'),
    url(r'^operate/$', OperateOrder.as_view(), name='operate'),
    url(r'^overview/$', RefundOverView.as_view(), name='refundoverview'),
    url(r'^upload/$', RefundUpLoad.as_view(), name='upload'),

]