...
class MyEditablePlugin(BaseAdminPlugin):
    ...
    def __init__(self, admin_view):
        super(MyEditablePlugin, self).__init__(admin_view)
        self.editable_need_fields = {}

    def init_request(self, *args, **kwargs):

        active = bool(self.request.method == 'GET' and self.admin_view.has_change_permission() and self.list_myeditable)
    ...
    def result_item(self, item, obj, field_name, row):
        ...
        if self.list_myeditable and item.field and item.field.editable and (field_name in self.list_myeditable) and row['object'].repeat_tag==1:
...
site.register_plugin(MyEditablePlugin, ListAdminView)
